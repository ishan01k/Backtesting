...
class ViewerHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def find_data_file(self, date_str):
        ...
        # Data folder lives one level up (at backtests root)
        data_dir = Path(__file__).parent.parent / "Data"
        if not data_dir.exists():
            data_dir = Path(__file__).parent / "Data"  # fallback
    ...
    def handle_day_summary(self, query):
        ...
        trades = []
        trades_file = Path(__file__).parent / "slope_all_trades_details.csv"
                ...
                with open(trades_file, newline='') as f:
                    ...
                    for row in reader:
                        if row.get('Date') == day:
                            ...
                            for k in ['Strike', 'Sell_Price', 'Exit_Price', 'Qty', 'Premium_Collected', 'PnL', 'Capital_After', 'Spot_945', 'Slope_CE', 'Slope_PE']:
        ...
        # Collect the strikes and legs that we want to track
        # For the slope strategy, we track BOTH CE and PE for the chosen ATM strike
        traded_options = {}
        for t in trades:
            try:
                strike = int(float(t.get("Strike", 0)))
                if strike > 0:
                    traded_options[f"{strike}_CE"] = []
                    traded_options[f"{strike}_PE"] = []
            except (ValueError, TypeError):
                continue

        # Extract unique spot price candles and option series
        spot_series = []
        seen_times = set()
        ...
        try:
            with open(csv_path, newline='') as f:
                ...
                for row in reader:
                    ...
                    # Collect option price series if this row belongs to a tracked contract
                    try:
...
def run_server(port=8503):
    server_address = ('', port)
    httpd = HTTPServer(server_address, ViewerHTTPRequestHandler)
    print(f"Starting options slope visualizer server on port {port}...")
    httpd.serve_forever()

# Global reference to server process state
_server_started = False

def start_server_process(port=8503):
...
def start_server_in_thread(port=8503):
